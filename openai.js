...
const initModule = async (openaiConf, apikey, assistantKey) => {
	...
	// If no assistant exist, then create one
	if (assistantKey === "") {
	...
	} else { // If assistant exists, then use it
		assistant = await openai.beta.assistants.retrieve(assistantKey);

		// Always update with latest tools and system
	...
	// debugQuery("Say hi to Constantin")
...
const createCompletion = async (userInput, chatId, ttsParams, assistantInstructions) => {
	...
	if (run.status === 'completed') {
		...
		for (let i = 0; i < run.required_action.submit_tool_outputs.tool_calls.length; i++) {
			let functionName = run.required_action.submit_tool_outputs.tool_calls[i].function.name;
			let args = run.required_action.submit_tool_outputs.tool_calls[i].function.arguments;
	...
	if (ttsParams.method === "elevenlabs") {
		...
		audio = await exportSpeech(response, ttsParams)
	} else {
